...
async function seed() {
  ...
  const parseUnitParts = (units) => {
    if (!units || typeof units !== 'string') return {};
    const match = units.match(/^\s*(\d+(?:\.\d+)?)\s*([a-zA-Z]+)\s*$/);
    if (!match) return {};
    return { quantity: parseFloat(match[1]), unit: match[2] };
  };

  const itemDocs = externalData.map(item => {
    const parsedUnits = parseUnitParts(item.units);
    const packageQuantity = item.unit_quantity ? parseFloat(item.unit_quantity) : parsedUnits.quantity;
    const packageUnit = parsedUnits.unit || null;
    ...
    return {
      ...
      ...(Number.isFinite(packageQuantity) ? { packageQuantity } : {}),
  ...
  RECIPE_SEEDS.forEach(recipe => {
    recipe.ingredients.forEach(ingredient => {
      if (ingredient.templateName) {
        recipeIngredientNames.add(ingredient.templateName);
      }
    });
  });

  const missingItemDocs = [];
  for (const name of recipeIngredientNames) {
    const normalized = normalizeName(name);
    if (!itemIdByNormalizedName.has(normalized)) {
      missingItemDocs.push({
        name,
        category: 'Other',
        tags: ['seeded', 'recipe'],
        createdAt: now,
  ...
  if (missingItemDocs.length > 0) {
    const missingInsert = await db.collection('items').insertMany(missingItemDocs);
  ...
  console.log(
    `Success! Seeded ${storeDocs.length} stores, ${uniqueItems.length} real items, and ${recipeDocs.length} recipes.`
  );
  await client.close();
}

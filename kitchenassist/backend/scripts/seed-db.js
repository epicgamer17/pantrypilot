...
async function seed() {
  ...
  // 1. Load the JSON data generated by the notebook
  const dataPath = path.join(__dirname, '../../../latest_grocery_data.json');
  ...
  const itemDocs = externalData.map(item => ({
    ...
    barcode: item.upc ? String(item.upc) : null, // Ensure upc is a string if it exists
    packageUnit: item.units || null,
